...
function ClientsList() {
  const [data, setData] = useState([
    ...
    {
      id: 4, // Agrega un id único para cada objeto de datos
      nombre: "Carlos Castro",
      fechaRegistro: "16/03/2023",
      status: "Inactivo",
    },{
      id: 5, // Agrega un id único para cada objeto de datos
      nombre: "Carlos Castro",
      fechaRegistro: "16/03/2023",
      status: "Inactivo",
    },{
      id: 6, // Agrega un id único para cada objeto de datos
      nombre: "Carlos Castro",
      fechaRegistro: "16/03/2023",
      status: "Inactivo",
    },
  ...
  const eliminarAppointment = (dato) => {
    Swal.fire({
      title: "¿Estás seguro de eliminar la reserva de " + dato.nombre + " en "+dato.lugar+"?",
      text: "¡No podrás revertir esto!",
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#3085d6",
  ...
  const keysClients = ["nombre", "status", "fechaRegistro"];
  const keysAppointment = ["nombre", "lugar", "fecha"];

  const searchClient = (info) => {
    return info.filter((item) =>
    keysClients.some((keysClients) => item[keysClients].toLowerCase().includes(query))
    );
  };

  const searchAppointment = (info) => {
    return info.filter((item) =>
    keysAppointment.some((keysAppointment) => item[keysAppointment].toLowerCase().includes(queryAppointment))
    );
  };
      ...
      <div>
        ...
        >
          <div className="contenedorTbl">
            ...
            <tbody>
              {searchClient(data).map((dato, index) => (
                <tr key={index}>
                  ...
                  <td style={{ paddingTop: "15px" }}>
                    ...
                    <Button color="danger" onClick={() => eliminarClient(dato)}>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="16"
                        height="16"
